...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		String wholeFile = FileUtils.readFileToString(file); // the whole file is written to a string
		String[] songs = wholeFile.split("\r\n"); //
		ArrayList<Song> list = new ArrayList<Song>();
		for (int i = 0; i < songs.length; i++) {
			list.add(new Song(songs[i]));
		}
		StringBuilder outputText = new StringBuilder(); // this will hold the output text
